...
@Component({
  selector: 'lib-container-job-tariff-labour-rate-list',
  templateUrl: './container-job-tariff-labour-rate-list.component.html',
  styleUrls: ['./container-job-tariff-labour-rate-list.component.scss'],
  providers: [
    DiscardingRequestQueue
  ]
})
export class ContainerJobTariffLabourRateListComponent implements OnInit, OnDestroy, OnChanges {
  ...
  private defaultColumns: DataGridColumn[] = [
    ...
    new DataGridColumn('amounts.Dry', 'Dry', '', DataGridColumn.TYPE_BIG_DECIMAL),
    new DataGridColumn('amounts.Reefer', 'Reefer', '', DataGridColumn.TYPE_BIG_DECIMAL),
    new DataGridColumn('amounts.Reefer Machinery', 'Reefer Machinery', '', DataGridColumn.TYPE_BIG_DECIMAL)
  ];
  ...
  refresh(): void {
    this.requestQueue.makeRequest(
      this.tariffService.getTariffs(this.pageSettings, this.filter, true),
      page => {
        if(page.content) {
